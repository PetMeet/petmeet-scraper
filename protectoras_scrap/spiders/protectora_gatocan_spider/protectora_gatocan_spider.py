...
def get_pet_type(url):
    return 'CAT' if 'G' in url else 'DOG'


# Get pet picture url
def get_pet_picture_url(base_url, response):
    return '/'.join([base_url, response.xpath(PROTECTORA_GATOCAN_PET_PICTURE_XPATH).get()])
...
class ProtectoraGatoCanSpider(scrapy.Spider):
    ...
    def parse_pet_urls(self, response):
        ...
        for url in pet_urls:
            yield scrapy.Request('/'.join([self.base_url, url]), callback=self.parse_pet_info,
                                 meta={'pet_type': get_pet_type(response.url)})
